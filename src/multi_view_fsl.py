...
class feature_proj(nn.Module):
    def __init__(self, hyp_params):
        ...
        # self.dim = 1024
        self.orig_d_1, self.orig_d_2, self.orig_d_3 = hyp_params.orig_d_l, hyp_params.orig_d_a, hyp_params.orig_d_v
        self.encoder_dim = hyp_params.encoder_dim

        # 1. Temporal convolutional layers

        # self.proj_1 = nn.Sequential(
        #     nn.Linear(self.orig_d_1, self.encoder_dim, bias=True),
        #     nn.ReLU(),
        #     nn.Dropout(0.1),
        ...
        #     nn.Linear(self.orig_d_2, self.encoder_dim, bias=True),
        #     nn.ReLU(),
        #     nn.Dropout(0.1),
        #     nn.Linear(self.encoder_dim, self.encoder_dim, bias=True),
        #     nn.ReLU(),
        ...
        #     nn.Linear(self.orig_d_3, self.encoder_dim, bias=True),
        #     nn.ReLU(),
        #     nn.Dropout(0.1),
        #     nn.Linear(self.encoder_dim, self.encoder_dim, bias=True),
        #     nn.ReLU(),
    ...
    def forward(self, x_1, x_2, x_3):
        ...
        if self.orig_d_1 == self.encoder_dim:
            proj_x_1, proj_x_2, proj_x_3 = x_1, x_2, x_3
        else:
            x_1, x_2, x_3 = x_1.to(torch.float32), x_2.to(torch.float32), x_3.to(torch.float32)
            proj_x_1, proj_x_2, proj_x_3 = self.proj_1(x_1), self.proj_2(x_2), self.proj_3(x_3)

        return proj_x_1, proj_x_2, proj_x_3
...
class ViewFormer(nn.Module):
    ...
    def forward(self, view1, view2, view3):
        view1_embedding, view2_embedding, view3_embedding = self.feature_projection(view1, view2, view3)
        # print('************', keywords_embedding.shape, actor_embedding.shape)
        mean_embedding = torch.cat((view1_embedding.unsqueeze(-2), view2_embedding.unsqueeze(-2), view3_embedding.unsqueeze(-2)), -2).mean(-2)
        x = torch.cat((view1_embedding.unsqueeze(-2), view2_embedding.unsqueeze(-2), view3_embedding.unsqueeze(-2)), -2)
        ...
        return view1_embedding, view2_embedding, view3_embedding, mean_embedding, fusion_embedding
