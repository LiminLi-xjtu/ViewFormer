...
class feature_proj(nn.Module):
    def __init__(self, hyp_params):
        ...
        # self.dim = 1024
        self.orig_d_1, self.orig_d_2, self.orig_d_3, self.orig_d_4 = hyp_params.orig_d_1, hyp_params.orig_d_2, hyp_params.orig_d_3, hyp_params.orig_d_4
    ...
    def forward(self, x_1, x_2, x_3, x_4):
        ...
        if self.orig_d_1 == self.encoder_dim:
            proj_x_1, proj_x_2, proj_x_3, proj_x_4 = x_1, x_2, x_3, x_4
        else:
            x_1, x_2, x_3, x_4 = x_1.to(torch.float32), x_2.to(torch.float32), x_3.to(torch.float32), x_4.to(torch.float32)
            proj_x_1, proj_x_2, proj_x_3, proj_x_4 = self.proj_1(x_1), self.proj_2(x_2), self.proj_3(x_3), self.proj_4(x_4)

        return proj_x_1, proj_x_2, proj_x_3, proj_x_4
...
class MULTModel(nn.Module):
    def __init__(self, hyp_params):
        ...
        self.pos = nn.Linear(4, self.d_l, bias=False)
    ...
    def forward(self, x):
        ...
        if self.view_encoding:
            ...
            mask2 = torch.Tensor((0, 1, 0, 0))
            mask3 = torch.Tensor((0, 0, 1, 0))
            mask4 = torch.Tensor((0, 0, 0, 1))
            encoding1 = self.pos(mask1)
            encoding2 = self.pos(mask2)
            encoding3 = self.pos(mask3)
            encoding4 = self.pos(mask4)
            position = torch.cat([encoding1, encoding2, encoding3, encoding4]).view(4,-1)
            # position = self.pos

        else:
            #print('false')
            position = torch.zeros(4, self.d_l, requires_grad=False)
...
class ViewFormer(nn.Module):
    ...
    def forward(self, view1, view2, view3, view4):
        view1_embedding, view2_embedding, view3_embedding, view4_embedding = self.feature_projection(view1, view2, view3, view4)
        # print('************', keywords_embedding.shape, actor_embedding.shape)
        mean_embedding = torch.cat((view1_embedding.unsqueeze(-2), view2_embedding.unsqueeze(-2), view3_embedding.unsqueeze(-2), view4_embedding.unsqueeze(-2)), -2).mean(-2)
        x = torch.cat((view1_embedding.unsqueeze(-2), view2_embedding.unsqueeze(-2), view3_embedding.unsqueeze(-2), view4_embedding.unsqueeze(-2)), -2)
        ...
        return view1_embedding, view2_embedding, view3_embedding, view4_embedding, mean_embedding, fusion_embedding
