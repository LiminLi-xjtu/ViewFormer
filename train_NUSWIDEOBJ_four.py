...
train_sampler = Sampler(multi_view_data.label, [0, 20], args.train_num, way, per_class)
val_sampler = Sampler(multi_view_data.label, [20, 25], args.val_num, way, per_class)
...
hyp_params = args
hyp_params.orig_d_1, hyp_params.orig_d_2, hyp_params.orig_d_3, hyp_params.orig_d_4 = 226, 145, 74, 129
# 65, 226, 145, 74, 129
...
def train_model(settings, hyp_params, train_loader, val_loader, test_loader):
    ...
    def equal_mean(view1, view2, view3, view4, batch_Z, net):

        eval_attr = batch_Z.squeeze(-1)

        if hyp_params.use_cuda:
            with torch.cuda.device(0):
                view1, view2, view3, view4, eval_attr = view1.cuda(), view2.cuda(), view3.cuda(), view4.cuda(), eval_attr.cuda()
        ...
        support_feature_view1, support_feature_view2, support_feature_view3, support_feature_view4, support_feature_mean, support_feature_cross = net(
            support_view1, support_view2, support_view3, support_view4)
        query_feature_view1, query_feature_view2, query_feature_view3, query_feature_view4, query_feature_mean, query_feature_cross = net(
            query_view1, query_view2, query_view3, query_view4)
        ...
        support_feature = torch.cat((support_feature_view1.unsqueeze(-2), support_feature_view2.unsqueeze(-2), support_feature_view3.unsqueeze(-2),
                                     support_feature_view4.unsqueeze(-2), support_feature_mean.unsqueeze(-2)), -2) #(shot,way,view+1,dim)
        query_feature = torch.cat((query_feature_view1.unsqueeze(-2), query_feature_view2.unsqueeze(-2), query_feature_view3.unsqueeze(-2),
                                   query_feature_view4.unsqueeze(-2), query_feature_mean.unsqueeze(-2)), -2)
        ...
        # 协同正则
        dis_co = distance[:, :, 0] + distance[:, :, 1] + distance[:, :, 2] + distance[:, :, 3]
    ...
    def train(model, optimizer, epoch, criterion):
        ...
        for [i_batch, (batch_X1, batch_X2, batch_X3, batch_X4, batch_X5, batch_Z)] in enumerate(train_loader):
            eval_attr = batch_Z.squeeze(-1)
            ...
            preds, query_label, cross_loss = equal_mean(batch_X2, batch_X3, batch_X4, batch_X5, batch_Z, net)
            ...
            if epoch <= pre_epoch:
                ...
                raw_loss = ((hyp_params.alpha_1 + 1) * view_1_loss + (hyp_params.alpha_2 + 1) * view_2_loss + (hyp_params.alpha_3 + 1) *
                            view_3_loss + (hyp_params.alpha_4 + 1) * view_4_loss) / (hyp_params.alpha_1 + hyp_params.alpha_2 + hyp_params.alpha_3 + hyp_params.alpha_4 + 4)
                ...
                view_3_loss = criterion(preds[:, :, 2], query_label)
                view_4_loss = criterion(preds[:, :, 3], query_label)
                raw_loss = (raw_loss + hyp_params.alpha_1 * view_1_loss + hyp_params.alpha_2 * view_2_loss + hyp_params.alpha_3 * view_3_loss + hyp_params.alpha_4 * view_4_loss) / (
                                   1 + hyp_params.alpha_1 + hyp_params.alpha_2 + hyp_params.alpha_3 + hyp_params.alpha_4)
            ...
            for [i_batch, (batch_X1, batch_X2, batch_X3, batch_X4, batch_X5, batch_Z)] in enumerate(loader):
                ...
                if epoch <= pre_epoch:
                    ...
                    raw_loss = ((hyp_params.alpha_1 + 1) * view_1_loss + (hyp_params.alpha_2 + 1) * view_2_loss +
                            ...
                            hyp_params.alpha_1 + hyp_params.alpha_2 + hyp_params.alpha_3 + hyp_params.alpha_4 + 4)
                    ...
                    view_3_loss = criterion(preds[:, :, 2], query_label)
                    view_4_loss = criterion(preds[:, :, 3], query_label)
                    raw_loss = (raw_loss + hyp_params.alpha_1 * view_1_loss + hyp_params.alpha_2 * view_2_loss + hyp_params.alpha_3 *
                                view_3_loss + hyp_params.alpha_4 *
                                view_4_loss) / (1 + hyp_params.alpha_1 + hyp_params.alpha_2 + hyp_params.alpha_3 + hyp_params.alpha_4)
    ...
    for epoch in range(1, hyp_params.num_epochs + 1):
        ...
        Acc_view1_val,  Acc_view2_val,  Acc_view3_val, Acc_view4_val, Acc_gap_val,  Acc_weight_val, Acc_co_val = 0, 0, 0, 0, 0, 0, 0
        Acc_view1_test, Acc_view2_test, Acc_view3_test, Acc_view4_test, Acc_gap_test, Acc_weight_test, Acc_co_test = 0, 0, 0, 0, 0, 0, 0
        ...
        print(
            'Epoch {:2d} | Time {:5.4f} sec | Train Loss {:5.4f} | Valid Loss {:5.4f} | Test Loss {:5.4f}'.format(epoch,
                                                                                                                  ...
                                                                                                                  test_loss))
        print(
            'train:   |  Accuracy_view1 {:5.4f} | Accuracy_view2 {:5.4f} | Accuracy_view3 {:5.4f} | Accuracy_view4 {:5.4f} |'
            '  Accuracy_gap {:5.4f} | Accuracy_att {:5.4f} | Accuracy_co {:5.4f} |'.format(
                Acc_view1_train,  Acc_view2_train, Acc_view3_train, Acc_view4_train, Acc_gap_train,  Acc_weight_train, Acc_co_train))
        print(
            'val:     |  Accuracy_view1 {:5.4f} | Accuracy_view2 {:5.4f} | Accuracy_view3 {:5.4f} | Accuracy_view4 {:5.4f} |'
            '  Accuracy_gap {:5.4f} | Accuracy_att {:5.4f} | Accuracy_co {:5.4f} |'.format(
                Acc_view1_val,  Acc_view2_val, Acc_view3_val, Acc_view4_val,  Acc_gap_val,  Acc_weight_val, Acc_co_val))
        print(
            'test:    |  Accuracy_view1 {:5.4f} | Accuracy_view2 {:5.4f} | Accuracy_view3 {:5.4f} | Accuracy_view4 {:5.4f} |'
            '  Accuracy_gap {:5.4f} | Accuracy_att {:5.4f} | Accuracy_co {:5.4f} |'.format(
                Acc_view1_test, Acc_view2_test, Acc_view3_test, Acc_view4_test, Acc_gap_test, Acc_weight_test, Acc_co_test))
        print(
            'improvment: \n',
            'valid:    |  view1 {:5.4f} | view2 {:5.4f} | view3 {:5.4f} | view4 {:5.4f} |  mean {:5.4f} \n'.format(
                (Acc_weight_val - Acc_view1_val) * 100, (Acc_weight_val - Acc_view2_val) * 100,
                (Acc_weight_val - Acc_view3_val) * 100, (Acc_weight_val - Acc_view4_val) * 100, (Acc_weight_val - Acc_gap_val) * 100),
            'test:     |  view1 {:5.4f} | view2 {:5.4f} | view3 {:5.4f} || view4 {:5.4f}   mean {:5.4f} '.format(
                impro_for_view1, impro_for_view2, impro_for_view3, impro_for_view4, impro_for_mean)
        )
    ...
    print(
        ...
        '  Accuracy_gap {:5.4f} | Accuracy_att {:5.4f} | Accuracy_co {:5.4f} |'.format(
            epo,Acc_view1_test, Acc_view2_test, Acc_view3_test, Acc_view4_test,  Acc_gap_test, Acc_weight_test, Acc_co_test))
    print(
        'improvment: \n'
        ' |  view1 {:5.4f} | view2 {:5.4f} | view3 {:5.4f} | view4 {:5.4f} |  mean {:5.4f} '.format(
            impro_for_view1, impro_for_view2, impro_for_view3, impro_for_view4, impro_for_mean))
