...
from src.five_view_fsl import feature_proj, MULTModel, ViewFormer
...
def train_model(settings, hyp_params, train_loader, val_loader, test_loader):
    ...
    def equal_mean(view1, view2, view3, view4, view5, batch_Z, net):
        ...
        support_feature = torch.cat((support_feature_view1.unsqueeze(-2), support_feature_view2.unsqueeze(-2), support_feature_view3.unsqueeze(-2),
                                     support_feature_view4.unsqueeze(-2), support_feature_view5.unsqueeze(-2), support_feature_mean.unsqueeze(-2)), -2) #(shot,way,view+1,dim)
        query_feature = torch.cat((query_feature_view1.unsqueeze(-2), query_feature_view2.unsqueeze(-2), query_feature_view3.unsqueeze(-2),
                                   query_feature_view4.unsqueeze(-2), query_feature_view5.unsqueeze(-2), query_feature_mean.unsqueeze(-2)), -2)
        ...
        # 协同正则
        dis_co = distance[:, :, 0] + distance[:, :, 1] + distance[:, :, 2] + distance[:, :, 3] + distance[:, :, 4]
    ...
    def train(model, optimizer, epoch, criterion):
        ...
        for [i_batch, (batch_X1, batch_X2, batch_X3, batch_X4, batch_X5, batch_Z)] in enumerate(train_loader):
            ...
            preds, query_label, cross_loss = equal_mean(batch_X1, batch_X2, batch_X3, batch_X4, batch_X5, batch_Z, net)
            ...
            if epoch <= pre_epoch:
                ...
                raw_loss = ((hyp_params.alpha_1 + 1) * view_1_loss + (hyp_params.alpha_2 + 1) * view_2_loss + (hyp_params.alpha_3 + 1) *
                            view_3_loss + (hyp_params.alpha_4 + 1) * view_4_loss+ (hyp_params.alpha_5 + 1) * view_5_loss) / (
                        hyp_params.alpha_1 + hyp_params.alpha_2 + hyp_params.alpha_3 + hyp_params.alpha_4+ hyp_params.alpha_5 + 5)
                ...
                raw_loss = (raw_loss + hyp_params.alpha_1 * view_1_loss + hyp_params.alpha_2 * view_2_loss + hyp_params.alpha_3 * view_3_loss
                            + hyp_params.alpha_4 * view_4_loss + hyp_params.alpha_5 * view_5_loss) / (
                                   1 + hyp_params.alpha_1 + hyp_params.alpha_2 + hyp_params.alpha_3 + hyp_params.alpha_4 + hyp_params.alpha_5)
            ...
            for [i_batch, (batch_X1, batch_X2, batch_X3, batch_X4, batch_X5, batch_Z)] in enumerate(loader):
                ...
                if epoch <= pre_epoch:
                    ...
                    raw_loss = ((hyp_params.alpha_1 + 1) * view_1_loss + (hyp_params.alpha_2 + 1) * view_2_loss +
                                ...
                                (hyp_params.alpha_5 + 1) * view_5_loss) / (
                            hyp_params.alpha_1 + hyp_params.alpha_2 + hyp_params.alpha_3 + hyp_params.alpha_4 + 5)
                    ...
                    view_4_loss = criterion(preds[:, :, 3], query_label)
                    view_5_loss = criterion(preds[:, :, 4], query_label)
                    raw_loss = (raw_loss + hyp_params.alpha_1 * view_1_loss + hyp_params.alpha_2 * view_2_loss +
                                hyp_params.alpha_3 * view_3_loss + hyp_params.alpha_4 * view_4_loss
                                + hyp_params.alpha_5 * view_5_loss) / (1 + hyp_params.alpha_1 + hyp_params.alpha_2 + hyp_params.alpha_3 + hyp_params.alpha_4 + hyp_params.alpha_5)
    ...
    print(
        ...
        ' |  Accuracy_view1 {:5.4f} | Accuracy_view2 {:5.4f} | Accuracy_view3 {:5.4f} | Accuracy_view4 {:5.4f} | Accuracy_view5 {:5.4f} |'
        '  Accuracy_gap {:5.4f} | Accuracy_att {:5.4f} | Accuracy_co {:5.4f} |'.format(
            epo,Acc_view1_test, Acc_view2_test, Acc_view3_test, Acc_view4_test,  Acc_view5_test,  Acc_gap_test, Acc_weight_test, Acc_co_test))
    print(
        'improvment: \n'
