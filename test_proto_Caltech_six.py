...
def test_model(settings, hyp_params, test_loader):
    ...
    def equal_mean(view1, view2, view3, view4, view5, view6, batch_Z, net):
        ...
        support_feature = torch.cat((support_feature_view1.unsqueeze(-2), support_feature_view2.unsqueeze(-2),
                                     ...
                                     support_feature_view4.unsqueeze(-2), support_feature_view5.unsqueeze(-2),
                                     support_feature_view6.unsqueeze(-2), support_feature_mean.unsqueeze(-2)),
                                    -2)  # (shot,way,view+1,dim)
        query_feature = torch.cat(
            (query_feature_view1.unsqueeze(-2), query_feature_view2.unsqueeze(-2), query_feature_view3.unsqueeze(-2),
             query_feature_view4.unsqueeze(-2), query_feature_view5.unsqueeze(-2), query_feature_view6.unsqueeze(-2),
    ...
    def evaluate(model, criterion, test=False):
        ...
        with torch.no_grad():
            for [i_batch, (batch_X1, batch_X2, batch_X3, batch_X4, batch_X5, batch_X6, batch_Z)] in enumerate(loader):
                ...
                net = model
                results, truths, _ = equal_mean(batch_X1, batch_X2, batch_X3, batch_X4, batch_X5, batch_X6, batch_Z, net)
