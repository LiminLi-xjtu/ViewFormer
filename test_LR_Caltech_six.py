...
def test_model(settings, hyp_params, test_loader):
    ...
    def equal_mean(view1, view2, view3, view4, view5, view6, batch_Z, net):
        ...
        support_feature_view1, support_feature_view2, support_feature_view3, \
        support_feature_view4, support_feature_view5, support_feature_view6, \
        support_feature_mean, support_feature_cross = \
            net(support_view1, support_view2, support_view3, support_view4, support_view5, support_view6)

        query_feature_view1, query_feature_view2, query_feature_view3, \
        query_feature_view4, query_feature_view5, query_feature_view6, \
        query_feature_mean, query_feature_cross = \
        ...
        support_feature_view4, support_feature_view5, support_feature_view6 = \
            ...
            support_feature_view6.view(shot * n_class, -1).cpu().numpy()


        query_feature_view1, query_feature_view2, query_feature_view3, \
        query_feature_view4, query_feature_view5, query_feature_view6 = \
            query_feature_view1.cpu().numpy(), query_feature_view2.cpu().numpy(), query_feature_view3.cpu().numpy(), \
            query_feature_view4.cpu().numpy(), query_feature_view5.cpu().numpy(), query_feature_view6.cpu().numpy()
        ...
        clf.fit(support_feature_view6, support_label)
        pred_6 = clf.predict_proba(query_feature_view6)
        ...
        pred_7 = clf.predict_proba(query_feature_mean)
        ...
        pred_8 = clf.predict_proba(query_feature_cross)

        pred_9 = (pred_1 + pred_2 + pred_3 + pred_4 + pred_5 + pred_6) / 6
        ...
        if torch.cuda.is_available():
            ...
            pred_9 = torch.from_numpy(pred_9).cuda()
        ...
        preds = torch.cat((pred_1.unsqueeze(-1), pred_2.unsqueeze(-1), pred_3.unsqueeze(-1),
                           ...
                           pred_7.unsqueeze(-1), pred_8.unsqueeze(-1), pred_9.unsqueeze(-1)), -1)
    ...
    def evaluate(model, criterion, test=False):
        ...
        with torch.no_grad():
            for [i_batch, (batch_X1, batch_X2, batch_X3, batch_X4, batch_X5, batch_X6, batch_Z)] in enumerate(loader):
                ...
                net = model
                results, truths = equal_mean(batch_X1, batch_X2, batch_X3, batch_X4, batch_X5, batch_X6, batch_Z, net)
            ...
            Acc_view6 = acc(preds[:, :, 5], label, True)

            Acc_mean = acc(preds[:, :, 6], label, True)
            Acc_cross = acc(preds[:, :, 7], label, True)
            Acc_co = acc(preds[:, :, 8], label, True)
            impro_for_view1 = (Acc_cross - Acc_view1) * 100
            impro_for_view2 = (Acc_cross - Acc_view2) * 100
            impro_for_view3 = (Acc_cross - Acc_view3) * 100
            impro_for_view4 = (Acc_cross - Acc_view4) * 100
            impro_for_view5 = (Acc_cross - Acc_view5) * 100
            impro_for_view6 = (Acc_cross - Acc_view6) * 100
            impro_for_mean = (Acc_cross - Acc_mean) * 100
            print("-" * 100)
            print('all task in test: \n'
                  ' |  Accuracy_view1 {:5.4f} | Accuracy_view2 {:5.4f} | Accuracy_view3 {:5.4f} | Accuracy_view4 {:5.4f} |'
                  ' Accuracy_view5 {:5.4f} | Accuracy_view6 {:5.4f} |Accuracy_gap {:5.4f} | Accuracy_cross {:5.4f} | Accuracy_co {:5.4f} |'.format(
                Acc_view1, Acc_view2, Acc_view3, Acc_view4, Acc_view5, Acc_view6, Acc_mean, Acc_cross, Acc_co))
            print(
                'improvment: \n'
                ' |  view1 {:5.4f} | view2 {:5.4f} | view3 {:5.4f} | view4 {:5.4f} | view5 {:5.4f} | view6 {:5.4f} |  mean {:5.4f} '.format(
                    ...
                    impro_for_view6, impro_for_mean))
